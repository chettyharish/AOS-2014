#Correct working of pattern removal and hash saving
#use strict;
use warnings;               #for displaying warnings
use Date::Parse;            #for converting date format
use Algorithm::Permute;     #for generating powersets with permutation
...
sub apriori {
	...
	my $support    = 0.005;    #minimum support
	...
	for ( $i = 2 ; $i <= $noofiterations ; $i++ ) {
		...
		if ( $i > 2 ) {
			...
			while ( my $c = $iter->next ) {
				...
				my $permute = new Algorithm::Permute( \@rowelements, $i - 1 );
				while ( @res = $permute->next ) {
					$pattern = "@res";
