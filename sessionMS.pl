...
my $inputlog         = "E:\\College\\Projects\\ZLearnPerl\\Small\\log_003";
...
sub apriori {
	...
	my $support    = 0.02;    #minimum support
	...
	foreach (@sorted) {
		...
		if ( $currentSupport >= $support ) {
			print OWRITER
			  "$reversehashtable{$_} => $counts{$_}	    HAS SUPPORT  ",
			  $currentSupport,
			  "\n";
	   ...
	   #Turning a apriorirow into individual elements and removing delimiter ~~~
		...
		foreach (@sorted) {
			...
			if ( $currentSupport >= $support ) {
				...
				#separating the elemts using delimiter ~~~ and unhashing
				@rowelements = split( "~~~", $_ );
				foreach (@rowelements) {
					$_ =~ s/~~~//g;
					$_ = trim($_);
				...
				print OWRITER "=> $counts{$_}	    HAS SUPPORT  ",
				  $currentSupport, "\n";
